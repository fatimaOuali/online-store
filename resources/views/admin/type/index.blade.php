    ...
    <table class="table table-bordered table-striped">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $type->getName() }}</td>
          <td>
            <a class="btn btn-primary" style="background-color: blueviolet; border-radius: 20px;" href="{{route('admin.type.edit', ['id'=> $type->getId()])}}">
              <i class="bi-pencil"></i>
            </a>
          </td>
          <td>
            <form action="{{ route('admin.type.delete', $type->getId())}}" method="POST">
              @csrf
              @method('DELETE')
