...
<div>
  <div class="mt-20">
    <h1 class="mb-10 text-center text-2xl font-bold">Favorises</h1>
    <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
      <div class="rounded-lg md:w-2/3">
        @if($viewData["favoris"]->count()>0)
        @foreach($viewData["favoris"] as $item)
        <div class="product_data justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
          @if ($item->products->productImage->count() > 0)
          <img src="{{asset($item->products->productImage->image)}}" alt="product-image" class="image w-full rounded-lg sm:w-40" />
      @endif
      
          <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
            <div class="mt-5 sm:mt-0">
              <h2 class="text-lg font-bold text-gray-900">{{$item->products->name}}</h2>
              
            </div>
            <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block ">
            
              <div class="flex justify-left border-gray-100">
               <button type="submit">
                <a href="{{ route('product.show', ['id'=> $item->products->getId()]) }}">
    ...
    <input type="hidden" class="product_id" value="{{$item->product_id}}">
                ...
                <span class="text-base font-normal text-red-500 line-through dark:text-gray-400">
                  ${{$item->products->sale}}</span>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="remove_wishlist h-5 w-5 cursor-pointer duration-150 hover:text-red-500">
...
<div class="bg-white flex items-center">
	<div class="container flex flex-col md:flex-row items-center justify-center px-5 text-gray-700">
          ...
          <p class="mb-8">Discover our products and add the ones you like to your wishlist.</p>
          
         <a href="{{ route('home.about') }}">
          <button class="px-4 inline py-2 text-sm font-medium leading-5 shadow text-white transition-colors duration-150 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-blue bg-blue-600 active:bg-blue-600 hover:bg-blue-700">add now</button>
        </a> 
        </div>
      <div class="max-w-lg">
