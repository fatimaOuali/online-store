  ...
  <div class="mt-20">
    ...
    @foreach ($order->getItems() as $item)
        <div class="product_data justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
          ...
          <img src="{{asset($item->getProduct()->productImage->image)}}" alt="product-image" class="image w-full rounded-lg sm:w-40" />
      @endif
            ...
            <div class="mt-5 sm:mt-0">
              <a  href="{{ route('product.show', ['id'=> $item->getProduct()->getId()]) }}">
                <h2 class="text-lg font-bold text-gray-900">
                   {{ $item->getProduct()->getName() }}</h2></a>
              <p class="mt-1 text-xs text-gray-700">
                ${{ $item->getPrice() }} <br> <span>{{  $order->getCreatedAt()  }}</span>
              </p>
            </div>
            <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block ">
              ...
              <div class="flex justify-left border-gray-100">
               <span>
                ...
                total :</b> ${{ $order->getTotal() +  4.99 }}
              </span>
            </div>

              <div class="flex items-center space-x-4"> 
                <a href="{{ route('myaccount.orders.delete', ['id' => $item->getId()]) }}">
...
<div class="bg-white flex items-center">
	<div class="container flex flex-col md:flex-row items-center justify-center px-5 text-gray-700">
   		<div class="max-w-md">
      		<div class="text-4xl font-dark font-bold">You have no orders.</div>

          <p class="mb-8">Discover our products and Order Now.</p>
          
         <a href="{{ route('home.about') }}"> <button class="px-4 inline py-2 text-sm font-medium leading-5 shadow text-white transition-colors duration-150 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-blue bg-blue-600 active:bg-blue-600 hover:bg-blue-700">Order Now</button>
         </a>
        </div>
      <div class="max-w-lg">
