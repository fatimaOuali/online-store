...
@foreach ($viewData["products"] as $product)
      ...
      @if ($product->productImage->count() > 0)
          <img src="{{ $product->productImage->image }}" alt="product-image" class="image w-full rounded-lg sm:w-40" />
      @endif
      
          <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
            <div class="mt-5 sm:mt-0">
              <h2 class="text-lg font-bold text-gray-900">{{ $product->getName() }}</h2>
              <p class="mt-1 text-xs text-gray-700">Size :{{ is_array(session('products')[$product->getId()]['size']) ? '' : session('products')[$product->getId()]['size'] }} </p>
            </div>
            <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
              <div class="flex items-center border-gray-100">
               <h3>
                Qty : {{ is_array(session('products')[$product->getId()]['quantity']) ? '' : session('products')[$product->getId()]['quantity'] }}
              </h3>
            </div>
              <div class="flex items-center space-x-4">
                <p class="text-sm">${{ $product->getPrice() }}
                  <span class="text-base font-normal text-red-500 line-through dark:text-gray-400">
                    ${{ $product->getSale()}}</span>
                  </p>
              <a href="{{ route('cart.deleteCartItem', ['product_id' => $product->id]) }}">
                <svg xmlns="http://www.w3.org/2000/svg"  fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="delete-item-btn h-5 w-5 cursor-pointer duration-150 hover:text-red-500">
...
@endforeach
      ...
      <div class="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
        <div class="mb-2 flex justify-between">
          ...
          <p class="text-gray-700">${{ $viewData["total"] }}</p>
        </div>
        ...
        <div class="flex justify-between">
          ...
          <div class="">
            <p class="mb-1 text-lg font-bold">${{ $viewData["total"] + 4.99 }} </p>
    ...
    <div class="bg-white flex items-center">
      <div class="container flex flex-col md:flex-row items-center justify-center px-5 text-gray-700">
           <div class="max-w-md">
              <div class="text-4xl font-dark font-bold">Nothing added yet</div>
    
              <p class="mb-8">Discover our products and order now.</p>
              
            <a href="{{ route('home.about') }}"> 
              <button class="px-4 inline py-2 text-sm font-medium leading-5 shadow text-white transition-colors duration-150 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-blue bg-blue-600 active:bg-blue-600 hover:bg-blue-700">Order now</button>
            </a> 
            </div>
          <div class="max-w-lg">
