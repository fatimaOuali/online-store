...
<div class="w-full max-w-sm bg-white border border-gray-200 w-60 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 hover:text-white mt-3">
    <a href="{{ route('product.show', ['id'=> $productItem->getId()]) }}">
        @if ($productItem->productImage->count() > 0)
        <img class="p-5 rounded-t-lg  h-70" src="/{{ $productItem->productImage->image }}" alt="product image" />
        @endif
    
    </a>
    <div class="px-2 pb-2 ">
        <a href="#">
            <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{{$productItem->getName() }}</h5>
        </a>
...
<div class="bg-white flex items-center">
	<div class="container flex flex-col md:flex-row items-center justify-center px-5 text-gray-700">
   		<div class="max-w-md">
      		<div class="text-4xl font-dark font-bold">Oops!!</div>

          <p class="mb-8">  There may be a misunderstanding in the name or the product might currently be unavailable.
            To verify the availability of the product, 
            please double-check the name or use different keywords in our website's search engine.
             If you need further assistance, feel free to contact our customer service team. </p>
          
         <a href="{{ route('message.index') }}"> <button class="px-4 inline py-2 text-sm font-medium leading-5 shadow text-white transition-colors duration-150 border border-transparent rounded-lg focus:outline-none focus:shadow-outline-blue bg-blue-600 active:bg-blue-600 hover:bg-blue-700">Contact Us</button>
         </a>
        </div>
      <div class="max-w-lg">
